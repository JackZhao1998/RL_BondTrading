...
def online_ps(epoch,gamma):
    ...
    for episode in range(epoch):
        ...
        for i in range(len(training_set_obs)-1):
            ...
            total_action_taken[str(i)]=[x_cord,y_cord,action_taken]
            ct_matrix[x_cord,y_cord,action_taken] += reward
            for x in range(dimension):
                for y in range(dimension):
                    for z in range(2):
                        as_matrix[x,y,z]=as_matrix[x,y,z]+ct_matrix[x,y,z]
        ...
        '''
        if episode != 0:
            if training_error < training_error_rate[-1]:
                for step in total_action_taken:
                    as_matrix[total_action_taken[step][0],total_action_taken[step][1],total_action_taken[step][2]] += 2
            else:
                for step in total_action_taken:
                    as_matrix[total_action_taken[step][0],total_action_taken[step][1],total_action_taken[step][2]] += -2
        '''
